...
class AuthService {
  ...
  constructor() {
    this.API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
  }

  async login(credentials: LoginCredentials): Promise<LoginResponse> {
    try {
      // Simulation pour le moment - remplacez par votre vraie API
...
export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-crm-purple via-crm-turquoise to-crm-pink">
        <div className="absolute -top-32 -left-32 w-64 h-64 bg-crm-pink rounded-full opacity-30 blur-[60px] animate-float"></div>
        <div className="absolute -bottom-32 -right-32 w-96 h-96 bg-crm-turquoise rounded-full opacity-30 blur-[60px] animate-float-delayed"></div>
        <div className="absolute top-1/2 left-[70%] w-56 h-56 bg-crm-purple rounded-full opacity-30 blur-[60px] animate-float-delayed-2"></div>
      </div>

      <div className="relative z-10 bg-white/95 backdrop-blur-xl rounded-3xl p-12 w-full max-w-md shadow-2xl animate-slideUp">
        <LoginForm />
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <p className="text-gray-600">Bienvenue dans votre CRM!</p>
        </div>
      </div>
    </div>
  );
